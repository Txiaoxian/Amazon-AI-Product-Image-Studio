...
- `P17-BE-STORAGE-QUOTA-RESERVATION` may add a tenant-scoped quota counter/reservation table. If added, it must include `tenant_id`, keep MySQL metadata as the reconciliation source of truth, and support atomic reservation/finalization/release for concurrent reference uploads and Worker outputs.
