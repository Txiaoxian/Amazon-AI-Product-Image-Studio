...
- `/admin/analytics/*` 聚合、详情和导出接口必须先校验租户管理员身份，再校验对应的`usage:read`、`user:read`或`audit:read`权限；所有查询都必须显式带有`tenant_id`范围。
- 不具备`usage:read`的管理员可以查看授权范围内的非费用统计，但不得看到预计费用、币种、费用状态或计费用量；CSV 导出遵循相同的脱敏规则。
- 管理统计默认只返回显示名称和稳定的技术 ID，不返回 Provider 原始请求/响应、API Key、Cookie、Authorization、图片 base64 或 MinIO 对象键。原始错误码仅在受权限保护的技术详情中保留。
- 后端导出限制最大行数并使用 UTF-8 BOM，避免浏览器端无限制拉取明细；时间范围和筛选值在路由边界校验，未知数据集不泄漏存在性。
