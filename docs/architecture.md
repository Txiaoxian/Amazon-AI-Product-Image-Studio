...
- The backend now exposes tenant `taskDefaults` and consumes them only when task creation omits both Provider/model IDs. Malformed persisted defaults are the immediate P13 hardening follow-up before further active settings are introduced.
...
Remaining follow-ups are documented in `docs/development-plan.md` and `docs/security.md`. P13 continues with runtime-default error hardening, then runtime-backed concurrency, quotas, and storage lifecycle, with the hard rule that writable settings cannot be exposed before their runtime consumers exist.
