...
1. `P11-BE-USER-ROLE-ADMIN`
   ...
   - 该任务需要先串行执行，因为它定义前端 admin UI 依赖的接口合同。
2. `P11-FE-USER-ROLE-ADMIN`
   - 前端用户与角色管理 UI。
   - 只能在后端任务 review 并合并后开始。
3. `R11`
   - 主 agent review 和回归。
...
除非用户选择其他顺序，否则下一个任务应生成 `P11-BE-USER-ROLE-ADMIN`。
...
- P11 先从后端任务串行开始。
- 在后端合同 review 并合并前，不要开始前端用户/角色管理。
